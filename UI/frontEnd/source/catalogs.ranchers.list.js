enyo.kind({
	...
	components:[
		...
		{kind: "Toolbar",	
		components: [					 
			{kind: "enyo.IconButton", flex:1,label:"Persona Fisica",onclick: "doPerson"},
			{kind: "enyo.IconButton", flex:1,label:"Empresa Sociedad",onclick: "doEnterprise"},
			{kind: "ListSelector", name: 'filter_id', label:"Filtro",hideItem:true,onChange: "filterRanchers",
			 items: [{caption:"Persona Fisica",value:1},
			        {caption:"Empresa/Sociedad",value:2},
					{caption:"Todo",value:3}] ,flex: 1,contentPack:"end"}					 
		]},		
	],
	...
	setupProductRow:function(inSender, inIndex) {		
		...
		if (objRan=this.objList[inIndex]) {
			...
			if(objRan.rancher_type==1){
				this.$.name.setContent(objRan.aka+" / "+objRan.first_name+" "+objRan.last_name);
	...
	updateList:function(){
		this.objList.sort(function(inA, inB) {			
			...
			return [inA['sortStr'], inA['sortStr']] < 
				   [inB['sortStr'], inB['sortStr']] ? -1 : 1;											
		});				
