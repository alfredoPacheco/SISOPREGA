...
public class BarnyardBean extends BaseBean implements Cruddable {
  ...
  public ReadGatewayResponse Read(GatewayRequest request) {
    ...
    Pen barnyard = null;
    try {
      barnyard = entityFromRequest(request, Pen.class);
      ...
      List<Pen> results = dataModel.readDataModelList(queryName, parameters, Pen.class);
      if (results.isEmpty()) {
        response.setError(new Error("VAL02", "No se encontraron datos para el filtro seleccionado", "proxy.BarnyardBean.Read"));
      } else {
        List<GatewayContent> records = contentFromList(results, Pen.class);
    ...
    try {
      ...
      if (barnyard.getBarnyardId() == 0) {
        ...
        if (validateEntity(barnyard)) {
          ...
          GatewayContent content = getContentFromEntity(barnyard, Pen.class);
