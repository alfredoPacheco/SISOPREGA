		...
		.kind({
			...
			get : function() {
				if (this.rancherWasReadFromGateway == false) {
					...
					// Retrieve Ranchers
					var cgReadAllRanchers = consumingGateway
							.Read("Rancher", {});
					if (cgReadAllRanchers.exceptionId == 0) { // Read
						...
						if (cgReadAllRanchers.exceptionId != "VAL02") { // No
							...
							cacheMan.setMessage("", "[Exception ID: "
									+ cgReadAllRanchers.exceptionId
									+ "] Descripción: "
									+ cgReadAllRanchers.exceptionDescription);
						}
					}
					// Retrieve Enterprise Ranchers:
					var cgReadAllEnterpriseRanchers = consumingGateway.Read(
							"EnterpriseRancher", {});
					if (cgReadAllEnterpriseRanchers.exceptionId == 0) { // Read
						// successfully
						jQuery
								.each(
										cgReadAllEnterpriseRanchers.records,
										function() {
						...
						if (cgReadAllEnterpriseRanchers.exceptionId != "VAL02") {// No
							...
							cacheMan
									.setMessage(
											"",
											"[Exception ID: "
													+ cgReadAllEnterpriseRanchers.exceptionId
													+ "] Descripcion: "
			...
			createEnterpriseRancher : function(objRan, cbObj, cbMethod) {
				...
				var cgCreateEnterpriseRancher = consumingGateway.Create(
						"EnterpriseRancher", objToSend);
				if (cgCreateEnterpriseRancher.exceptionId == 0) { // Created
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgCreateEnterpriseRancher.exceptionId
							+ "] Descripcion: "
			...
			upd : function(objOld, objNew, cbObj, cbMethod) {
				if (objOld.rancher_type == 1) {
					...
					return this.updateEnterpriseRancher(objOld, objNew, cbObj,
							cbMethod);
				}
			},
			updateRancher : function(objOld, objNew, cbObj, cbMethod) {
				...
				var cgUpdateRancher = consumingGateway.Update("Rancher",
						objToSend);
				if (cgUpdateRancher.exceptionId == 0) { // Updated successfully
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgUpdateRancher.exceptionId + "] Descripcion: "
							+ cgUpdateRancher.exceptionDescription);
					return false;
				}
			},
			updateEnterpriseRancher : function(objOld, objNew, cbObj, cbMethod) {
				...
				var cgUpdateEnterpriseRancher = consumingGateway.Update(
						"EnterpriseRancher", objToSend);
				if (cgUpdateEnterpriseRancher.exceptionId == 0) { // Updated
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgUpdateEnterpriseRancher.exceptionId
							+ "] Descripcion: "
			...
			del : function(delObj, cbObj, cbMethod) {
				if (delObj.rancher_type == 1) {
					...
					return this
							.deleteEnterpriseRancher(delObj, cbObj, cbMethod);
				}
			},
			deleteRancher : function(delObj, cbObj, cbMethod) {
				...
				var cgDeleteRancher = consumingGateway.Delete("Rancher",
						objToSend);
				if (cgDeleteRancher.exceptionId == 0) { // Deleted successfully
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgDeleteRancher.exceptionId + "] Descripcion: "
							+ cgDeleteRancher.exceptionDescription);
					return false;
				}
			},
			deleteEnterpriseRancher : function(delObj, cbObj, cbMethod) {
				...
				var cgDeleteEnterpriseRancher = consumingGateway.Delete(
						"EnterpriseRancher", objToSend);
				if (cgDeleteEnterpriseRancher.exceptionId == 0) { // Deleted
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgDeleteEnterpriseRancher.exceptionId
							+ "] Descripcion: "
			...
			getContacts : function(objRan) {
				...
				if (objRan.rancher_type == 1) {
					...
					cgReadContacts = consumingGateway.Read("RancherContact",
							objToSend);
					if (cgReadContacts.exceptionId == 0) { // Read successfully
						jQuery.each(cgReadContacts.records, function() {
							...
							arrAux.push(selfCacheRancher
									.rancherContactAdapterToIn(objAux));
						});
					}
					return objRan.contacts = arrAux;
				} else if (objRan.rancher_type == 2) {
					objToSend.enterpriseId = objRan.rancher_id;
					cgReadContacts = consumingGateway.Read("EnterpriseContact",
							objToSend);
					if (cgReadContacts.exceptionId == 0) { // Read successfully
						jQuery
								.each(
										cgReadContacts.records,
										function() {
											jQuery.each(this, function(key,
													value) {
												objAux[key] = value;
											...
											arrAux
													.push(selfCacheRancher
															.enterpriseRancherContactAdapterToIn(objAux));
										});
					}
					return objRan.contacts = arrAux;
				}
			...
			addContact : function(objRancher, objCon, cbObj, cbMethod) {
				...
				if (objRancher.rancher_type == 1) {
					...
					cgCreateContact = consumingGateway.Create("RancherContact",
							objToSend);
				} else if (objRancher.rancher_type == 2) {
					objToSend = this
							.enterpriseRancherContactAdapterToOut(objCon);
					delete objToSend.contactId;
					cgCreateContact = consumingGateway.Create(
							"EnterpriseContact", objToSend);
				}
				...
				if (cgCreateContact.exceptionId == 0) { // Created successfully
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgCreateContact.exceptionId + "] Descripcion: "
							+ cgCreateContact.exceptionDescription);
					return false;
				}

			},
			updateContact : function(objRancher, objOld, objNew, cbObj,
					cbMethod) {
				var objToSend = {};
				...
				if (objRancher.rancher_type == 1) {
					...
					cgUpdateContact = consumingGateway.Update("RancherContact",
							objToSend);
				} else if (objRancher.rancher_type == 2) {
					objToSend = this
							.enterpriseRancherContactAdapterToOut(objNew);
					cgUpdateContact = consumingGateway.Update(
							"EnterpriseContact", objToSend);
				}
				...
				if (cgUpdateContact.exceptionId == 0) { // Updated successfully
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgUpdateContact.exceptionId + "] Descripcion: "
							+ cgUpdateContact.exceptionDescription);
					return false;
				}
			},
			deleteContact : function(objRancher, objCon, cbObj, cbMethod) {
				...
				if (objRancher.rancher_type == 1) {
					cgDeleteContact = consumingGateway.Delete("RancherContact",
							objToSend);
				} else if (objRancher.rancher_type == 2) {
					cgDeleteContact = consumingGateway.Delete(
							"EnterpriseContact", objToSend);
				}
				...
				if (cgDeleteContact.exceptionId == 0) { // Deleted successfully
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgDeleteContact.exceptionId + "] Descripcion: "
							+ cgDeleteContact.exceptionDescription);
					return false;
				}
			},
			getBillings : function(objRan) {
				...
				objToSend.rancherId = objRan.rancher_id;
				cgReadBillings = consumingGateway.Read("RancherInvoice",
						objToSend);
				if (cgReadBillings.exceptionId == 0) { // Read successfully
					jQuery.each(cgReadBillings.records, function() {
						...
						arrAux.push(selfCacheRancher
								.rancherBillingAdapterToIn(objAux));
					});
				}
			...
			addBilling : function(objRancher, objCon, cbObj, cbMethod) {
				...
				if (cgCreateBilling.exceptionId == 0) { // Created successfully
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgCreateBilling.exceptionId + "] Descripcion: "
							+ cgCreateBilling.exceptionDescription);
					return false;
				}

			},
			updateBilling : function(objRancher, objOld, objNew, cbObj,
					cbMethod) {
				var objToSend = {};
				...
				cgUpdateBilling = consumingGateway.Update("RancherInvoice",
						objToSend);
				...
				if (cgUpdateBilling.exceptionId == 0) { // Updated successfully
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgUpdateBilling.exceptionId + "] Descripcion: "
							+ cgUpdateBilling.exceptionDescription);
					return false;
				}
			},
			deleteBilling : function(objRancher, objCon, cbObj, cbMethod) {
				...
				if (cgDeleteBilling.exceptionId == 0) { // Deleted successfully
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgDeletebilling.exceptionId + "] Descripcion: "
							+ cgDeletebilling.exceptionDescription);
					return false;
				}
			},
