		...
		.kind({
			...
			receptionAdapterToIn : function(objReception) {
				...
				if (rancherAux.rancher_type == 1) {
					objNew.rancher_name = rancherAux.first_name + " "
							+ rancherAux.last_name;
				} else {
			...
			get : function() {
				if (this.arrObjWasFilledUpOnce == false) {
					...
					for ( var a in this.arrReception) {
						...
						// barnyards::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
						var barnyardAux = this
								.getReceptionBarnyard(objAux.reception_id);
						...
						// weight:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
						var arrHeadcountAux = this
								.getReceptionHeadcount(objAux.reception_id);
						...
						var arrInspectionAux = this
								.getInspection(objAux.reception_id);
						for (i in arrInspectionAux) {
							...
							// inspections
							// Details::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
							var arrInspectionDetailsAux = this
									.getInspectionDetails(inspectionAux.rejected_id);
						...
						for (f in arrFeedAux) {
							...
							if (feedAux.feeding_id) {
								var arrFeedBarnyardAux = this
										.getFeedOrderBarnyard(feedAux.feeding_id);
								var feedBarnyardAux = {
									barnyards : {}
								};
								for (fb in arrFeedBarnyardAux) {
									var barnyardName = cacheBY
											.getByID(arrFeedBarnyardAux[fb].barnyardId).barnyard_code;
									feedBarnyardAux.barnyards[barnyardName] = barnyardName;
								}
								feedAux.barnyards = feedBarnyardAux.barnyards;
								// feedOrderDetails:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
								var arrFeedDetailsAux = this
										.getFeedOrderDetails(feedAux.feeding_id);
								var feedDetailsAux = {};
								var contAux = 1;
								for (fd in arrFeedDetailsAux) {
									feedDetailsAux[contAux] = {};
									feedDetailsAux[contAux].feed_desc = cacheFeed
				...
				if (this.receptionWasReadFromGateway == false) {
					...
					if (cgReadAll.exceptionId == 0) { // Read successfully
						for (item in cgReadAll.records) {
							arrAux
									.push(this
											.receptionAdapterToIn(cgReadAll.records[item]));
						}
					} else { // Error
						if (cgReadAll.exceptionId != "VAL02") { // No data found
							cacheMan.setMessage("", "[Exception ID: "
									+ cgReadAll.exceptionId + "] Descripción: "
			...
			getReceptionBarnyard : function(recID) {
				...
				var cgReadAll = consumingGateway.Read("ReceptionBarnyard",
						objToSend);
				...
				if (cgReadAll.exceptionId == 0) { // Read successfully
					for (item in cgReadAll.records) {
						arrAux
								.push(this
										.receptionBarnyardAdapterToIn(cgReadAll.records[item]));
			...
			getReceptionHeadcount : function(recID) {
				...
				var objToSend = {};
				objToSend.receptionId = recID;
				var cgReadAll = consumingGateway.Read("ReceptionHeadcount",
						objToSend);
				...
				if (cgReadAll.exceptionId == 0) { // Read successfully
					for (item in cgReadAll.records) {
						arrAux
								.push(this
										.receptionHeadcountAdapterToIn(cgReadAll.records[item]));
					}
				} else { // Error
					if (cgReadAll.exceptionId != "VAL02") { // No data found
						cacheMan.setMessage("", "[Exception ID: "
			...
			getFeedOrder : function(recID) {
				...
				if (cgReadAll.exceptionId == 0) { // Read successfully
					for (item in cgReadAll.records) {
						arrAux.push(this
								.feedOrderAdapterToIn(cgReadAll.records[item]));
					}
				} else { // Error
					if (cgReadAll.exceptionId != "VAL02") { // No data found =
															// VAL02 for filter
			...
			getFeedOrderBarnyard : function(oID) {
				...
				var cgReadAll = consumingGateway.Read("FeedOrderBarnyard",
						objToSend);
				...
				if (cgReadAll.exceptionId == 0) { // Read successfully
					for (item in cgReadAll.records) {
						arrAux
								.push(this
										.feedOrderBarnyardAdapterToIn(cgReadAll.records[item]));
					}
				} else { // Error
					if (cgReadAll.exceptionId != "VAL02") { // No data found
						cacheMan.setMessage("", "[Exception ID: "
			...
			getFeedOrderDetails : function(oID) {
				...
				var objToSend = {};
				objToSend.orderId = oID;
				var cgReadAll = consumingGateway.Read("FeedOrderDetails",
						objToSend);
				...
				if (cgReadAll.exceptionId == 0) { // Read successfully
					for (item in cgReadAll.records) {
						arrAux
								.push(this
										.feedOrderDetailsAdapterToIn(cgReadAll.records[item]));
					}
				} else { // Error
					if (cgReadAll.exceptionId != "VAL02") { // No data found
						cacheMan.setMessage("", "[Exception ID: "
			...
			getInspection : function(recID) {
				...
				if (cgReadAll.exceptionId == 0) { // Read successfully
					for (item in cgReadAll.records) {
						arrAux
								.push(this
										.inspectionAdapterToIn(cgReadAll.records[item]));
					}
				} else { // Error
					if (cgReadAll.exceptionId != "VAL02") { // No data found =
															// VAL02 for filter
			...
			getInspectionBarnyard : function(recID) {
				...
				var objToSend = {};
				objToSend.orderId = oID;
				var cgReadAll = consumingGateway.Read("InspectionBarnyard",
						objToSend);
				...
				if (cgReadAll.exceptionId == 0) { // Read successfully
					for (item in cgReadAll.records) {
						arrAux
								.push(this
										.inspectionBarnyardAdapterToIn(cgReadAll.records[item]));
					}
				} else { // Error
					if (cgReadAll.exceptionId != "VAL02") { // No data found
						cacheMan.setMessage("", "[Exception ID: "
			...
			getInspectionDetails : function(inspID) {
				...
				var cgReadAll = consumingGateway.Read("InspectionDetails",
						objToSend);
				...
				if (cgReadAll.exceptionId == 0) { // Read successfully
					for (item in cgReadAll.records) {
						arrAux
								.push(this
										.inspectionDetailsAdapterToIn(cgReadAll.records[item]));
					}
				} else { // Error
					if (cgReadAll.exceptionId != "VAL02") { // No data found
						cacheMan.setMessage("", "[Exception ID: "
			...
			Create : function(objRec, cbObj, cbMethod) {
				...
				if (cgCreate.exceptionId == 0) { // Created successfully
					...
					var today_sf = month + '/' + today.getDate() + '/'
							+ today.getFullYear();
					var report_name = 'RecibidoPorGanadero?rancherId='
							+ objRec.rancher_id + '&amp;fromDate=' + today_sf
							+ '&amp;toDate=' + today_sf;
			...
			upd : function(objOld, objNew, cbObj, cbMethod) {
				...
				if (cgUpdate.exceptionId == 0) { // Updated successfully
					...
					// TODO UPDATE WEIGHT
					if (this.updateWeight(objOld.weights[0], objNew.weights[0],
							cbObj) == true) {
					...
					cacheMan.setMessage("", "[Exception ID: "
							+ cgUpdate.exceptionId + "] Descripcion: "
							+ cgUpdate.exceptionDescription);
					return false;
				}
			},
			getLS : function() {
			...
			createWeight : function(receptionID, objWeight) {
				...
				var cgCreate = consumingGateway.Create("ReceptionHeadcount",
						objToSend);
				if (cgCreate.exceptionId == 0) { // Created successfully
			...
			updateWeight : function(objOld, objNew, cbObj) {
				...
				var cgUpdate = consumingGateway.Update("ReceptionHeadcount",
						objToSend);
				if (cgUpdate.exceptionId == 0) { // Updated successfully
					for (prop in objNew) {
						objOld[prop] = objNew[prop];
			...
			createFeedOrder : function(objRec, objFeed) {
				...
				if (cgCreate.exceptionId == 0) { // Created successfully
					...
					if (cacheReceptions.createFeedOrderBarnyard(
							cgCreate.generatedId, objFeed) == true) {
						if (cacheReceptions.createFeedOrderDetails(
								cgCreate.generatedId, objFeed) == true) {
							return true;
						} else {
							return false;
			...
			createFeedOrderBarnyard : function(order_id, objFeed) {
				...
				for (prop in objFeed.barnyards) {
					objToSend.barnyardId = cacheBY
							.getByBarnyard(objFeed.barnyards[prop]).barnyard_id;
				}
				var cgCreate = consumingGateway.Create("FeedOrderBarnyard",
						objToSend);
				if (cgCreate.exceptionId == 0) { // Created successfully
					return true;
				} else { // Error
			...
			createFeedOrderDetails : function(order_id, objFeed) {
				...
				for (obj in objFeed.feed) {
					...
					var cgCreate = consumingGateway.Create("FeedOrderDetails",
							objToSend);
			...
			updateFeedOrderDetails : function(order_id, objOld, objNew) {
				...
				for (obj in objNew.feed) {
					...
					var cgUpdate = consumingGateway.Update("FeedOrderDetails",
							objToSend);
			...
			updateFeed : function(objOld, objNew, cbObj, cbMethod) {
				...
				if (cgUpdate.exceptionId == 0) { // Updated successfully
					this.updateFeedOrderDetails(objOld.feeding_id, objOld,
							objNew) == true;
					for (prop in objNew) {
						objOld[prop] = objNew[prop];
					}
			...
			createInspectionBarnyard : function(inspection_id, objRec, objRej) {
				...
				for (prop in objRec.barnyards) {
					objToSend.barnyardId = cacheBY
							.getByBarnyard(objRec.barnyards[prop]).barnyard_id;
					var cgCreate = consumingGateway.Create(
							"InspectionBarnyard", objToSend);
					if (cgCreate.exceptionId != 0) { // Created successfully
						cacheMan.setMessage("", "[Exception ID: "
								+ cgCreate.exceptionId + "] Descripcion: "
								+ cgCreate.exceptionDescription);
			...
			createInspectionDetails : function(inspection_id, objRec, objRej) {
				...
				var cgCreate = consumingGateway.Create("InspectionDetails",
						objToSend);
				if (cgCreate.exceptionId == 0) {
			...
			updateReject : function(objRec, iInspIdx, objReject, cbObj,
					cbMethod) {
				var objToSend = {};
				...
				var cgUpdate = consumingGateway.Update("InspectionDetails",
						objToSend);
				if (cgUpdate.exceptionId == 0) { // Updated successfully
			...
			deleteReject : function(arrRejects, objReject, cbObj, cbMethod) {
				...
				var cgDelete = consumingGateway.Delete("InspectionDetails",
						objToSend);
				if (cgDelete.exceptionId == 0) { // Deleted successfully
