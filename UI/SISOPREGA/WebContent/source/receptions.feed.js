enyo.kind({
	...
	components:[
		{kind: "Popup",name: "manejo", lazy:false,
		 ...
		 openClassName: "zoomFadeIn", className: "transitioner2", 
		 layoutKind: "VFlexLayout",width: "90%",
		 ...
		 components:[
			...
			{kind: "Button",className: "enyo-button-affirmative",
    		 caption: "OK",onclick:"closeHandling"}]}
			]},	
							...
							{kind: enyo.SwipeableItem,
								...
								components: [
									...
									{layoutKind: enyo.HFlexLayout,components:[
										{name: "lblground",flex:1,
										 feed_id:1, 
										 className:"listSecond",
										 content: "MOLIDA(0)"},
										{name: "lblalfalfa",flex:1,
										 feed_id:2, 
										 className:"listSecond",
										 content: "ALFALFA(0)"},	
										{name: "lbloat",flex:1,
									...
									{name: "lblhandling",className:"listSecond",
									content:""}																		
								]
							},
						]},						
		...
		{kind: "Toolbar",	
		components: [					 
		{kind: "ToolInput", name:"ground",flex:1,  width:"10%", inputType:"number", 
		 hint:"Molida",changeOnInput: true,},			
		{kind: "ToolInput", name:"alfalfa",flex:1, width:"10%", inputType:"number",
		 hint:"Alfalfa",changeOnInput: true,},							
		{kind: "ToolInput", name:"oat",flex:1, width:"10%", inputType:"number",
		 hint:"Avena",changeOnInput: true,},							
		{kind: "ToolInput", name:"concentrated",flex:1.3, width:"12%", inputType:"number", 
		 hint:"Concentrado",changeOnInput: true,},	
		{kind: "ToolInput", name:"corn",flex:1, width:"10%",inputType:"number",
		 hint:"Maiz",changeOnInput: true,},
		{kind: "Spacer"},
		...
		{kind: "Drawer", name:"draUpdate", components: [		
			{layoutKind: "HFlexLayout", align: "center",components: [			
				...
				icon:"images/btn_edit.png",
				 flex:1, onclick: "updateFeedHandling"},							
		...
		if (this._objRec.feed){
			if (objFeed=this._objRec.feed[inIndex]){
				...
				if(objFeedfeed=objFeed.feed[this.$.lblalfalfa.feed_id]){
					if(objFeedfeed.feed_units){					
						this.$.lblalfalfa.setContent(objFeedfeed.feed_desc+
												 " ( "+objFeedfeed.feed_units+" )");
					}
				}
				if(objFeedfeed=objFeed.feed[this.$.lbloat.feed_id]){				
					if(objFeedfeed.feed_units){				
					this.$.lbloat.setContent(objFeedfeed.feed_desc+
												 " ( "+objFeedfeed.feed_units+" )");
					}
				}
				if(objFeedfeed=objFeed.feed[this.$.lblconcentrated.feed_id]){								
					if(objFeedfeed.feed_units){				
						this.$.lblconcentrated.setContent(objFeedfeed.feed_desc+
												 " ( "+objFeedfeed.feed_units+" )");
					}
				}
				if(objFeedfeed=objFeed.feed[this.$.lblcorn.feed_id]){												
					if(objFeedfeed.feed_units){
					this.$.lblcorn.setContent(objFeedfeed.feed_desc+
												 " ( "+objFeedfeed.feed_units+" )");
					}
				}							 											 											 				return true;
			}
	...
	getFeed:function(sOp){
		var objData={feeding_id:null,barnyards:{},feed:{},handling:""};
		
		var objFeed={feed_id:"",feed_desc:"",feed_units:""};		
		objFeed.feed_id=1;
		...
		objFeed={feed_id:"",feed_desc:"",feed_units:""};
		objFeed.feed_id=2;
		objFeed.feed_desc=cacheFeed.getByID(objFeed.feed_id).feed_desc;
