enyo.kind({
	...
	cellOver:function(inSender, inEvent){
		if(inSender.occupied!=0 && inSender.occupied!=2){
			this.highLightReception(crudPen.inUse()[inSender.name].receptionId);
		}
	},
		...
		switch(inSender.occupied){
			...
			case 1: //Seleccionar corral ocupado
				...
				if(enyo.json.stringify(this.arrSelected)!="{}"){
					...
					for(var sKey in this.arrSelectedOccupied){
						if(crudPen.inUse()[sKey].receptionId!=crudPen.inUse()[inSender.name].receptionId){
							for(var sKey in this.arrSelectedOccupied){
								this.$[sKey].occupied=1;
					...
					this.colorBYbyRancherSelected(crudReception.getByID(crudPen.getRecIDbyBY(inSender.name)).rancherId);
			...
			case 3://Deseleccionar corral ocupado
				...
				this.objSelected.applyStyle("background-color",crudReception.getByID(crudPen.getRecIDbyBY(this.objSelected.name)).color);									
				break;								
...
//							this.$[sKey].applyStyle("background-color",crudReception.getByID(crudPen.getRecIDbyBY(sKey)).color);
	...
	setDesc:function(sBY){
		...
		try {
		var objRec=crudReception.getByID(crudPen.inUse()[sBY].receptionId);
		var sBy="";
		var iBy=0;
...
//			  window.focus();
	...
	rancherFilterChanged:function(inSender){
		if(this.$.rancherFilter.getIndex()>-1){
			var receptions = crudReception.getActiveBYForListByRancherID(this.$.rancherFilter.getIndex());
				...
				for (var i = 0, a; (a=this.$.cells.children[i]); i++) {
					for (var j = 0, b; (b =a.children[j]); j++) {
						if(b.bBY==true){
							...
							for(x in receptions){
								var byFinded = crudPen.getByBarnyard(b.name);
								if(byFinded){
									if(byFinded.barnyard_id == receptions[x].value){
										this.$[b.name].occupied=1;										
										this.$[b.name].applyStyle("background-color",crudReception.getByID(crudPen.getRecIDbyBY(b.name)).color);
										break;
									}
								}
							}
	...
	colorBYbyRancherSelected:function(rancher_id){
		this.arrBYbyRancherSelected = crudReception.getActiveBYForListByRancherID(rancher_id);
		
		for(i in this.arrBYbyRancherSelected){
			var activeBY = "" + this.arrBYbyRancherSelected[i].zone_id + this.arrBYbyRancherSelected[i].barnyard_code;
			this.$[activeBY].applyStyle("background-color",crudReception.getByID(crudPen.getRecIDbyBY(activeBY)).color);
		}
