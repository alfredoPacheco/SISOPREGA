...
//	}
	...
	this.beforeSave(updateObject);
	
	entityKind.update(obj,this, "afterUpdateEntity", this.updatingEntityId);
...
//		"afterUpdateEntity", this.updatingEntityId);
	...
	if (updateResult.exceptionId != 0) {
	    cacheMan.hideScrim();
	    cacheMan.setMessage("", "Error al actualizar datos: "
		    + updateResult.exceptionDescription + ".");
	} else {
	    this.resetValues();
	    this.doUpdateEntity();
	    cacheMan.hideScrim();
	...
	var lowerCaseFirstChar = this.entityKind.entityName.substring(0, 1).toLowerCase();
