enyo
	.kind(
	{
	  ...
	  loadAutocompletes : function() {
		...
		this.$.consignee.setItems(crudConsignee.getList());
		this.$.details.$.penAutoComplete.setItems(crudPen.getListUsaPens());
		this.$.details.$.classAutoComplete
			.setItems(crudCattleQuality.getList());
		this.$.details.$.classAutoCompleteExpo.setItems(crudCattleQuality
			.getList());
	  ...
	  showConsigneeForm : function() {
		if (this.$.consigneeForm) {
		  this.$.consigneeForm.destroy();
		}
		this.$.addConsigneeDialog.createComponent(
		{
		  kind : "catalogs.consignee.form",
		  onAdd : "addConsignee",
		  onCancel : "cancelCreateConsignee",
		  name : "consigneeForm",
		  flex : 1
		},
		{
		...
		this.$.addConsigneeDialog.render();
		this.$.addConsigneeDialog.openAtCenter();
	  },
	  addConsignee : function(inSender, result) {
		this.$.consignee.setItems(crudConsignee.getList());

		var justCreated = result.records[0];
		this.$.consignee.setIndex(justCreated.consigneeId);
		this.$.addConsigneeDialog.close();
		cacheMan.hideScrim();
	  },
	  cancelCreateConsignee : function() {
		this.$.addConsigneeDialog.close();
	  },
	  printHermana : function() {
		if (this.hermanaId) {
		  ...
		  cacheMan.setMessage('',
			  'This record has not been saved on the database yet.');
		}
	  },
	  ...
	  printAgencia : function() {
		this.$.popup_print.close();
		utils.openReport('/ReportingGateway/Hermana_agencia?Id='
			+ this.hermanaId);
	  },
	  sendSMS : function() {
		// Send report
		if (this.hermanaId) {
		  ...
		  cacheMan.setMessage('',
			  'This record has not been saved on database yet.');
		}
	  },
	  resetForm : function() {
	  ...
	  openHermana : function(sender, selectedItem) {
		...
		this.$.details.setCattleClass(selectedItem.cattleClass,
			selectedItem.cattleClassName);
	  ...
	  saveHermana : function() {
		...
		for ( var i = 0; i < this.$.details.$.chargeList.arrData.length; i++) {
		  var expense = this
			  .ExpenseOutput(this.$.details.$.chargeList.arrData[i]);
	  ...
	  createCallBack : function(result) {
		...
		if (result.exceptionId != '0') {
		  cacheMan
			  .setMessage('',
				  'An error has occurred during save process, please check your input.');
		  cacheMan.hideScrim();
		  return false;
		}
		...
		utils.setCookie("expenses", this
			.chargesIndexString(this.$.details.$.chargeList.arrData), 15);
